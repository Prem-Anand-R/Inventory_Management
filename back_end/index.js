...
app.post('/login', async (req, res) => {
    ...
    try {
        const result = await db.query('SELECT * FROM login_page WHERE user_name = $1 AND password = $2', [username, password]);
...
const buildDateRangeQuery = (startDate, endDate) =>
    startDate && endDate ? `TO_DATE(date, 'YYYY-MM-DD') BETWEEN '${startDate}' AND '${endDate}'` : '';
...
const lowercaseProductName = productName ? productName.toLowerCase() : '';
   return startDate && endDate && lowercaseProductName
        ? `TO_DATE(date, 'YYYY-MM-DD') BETWEEN '${startDate}' AND '${endDate}' AND LOWER(product_name) = '${lowercaseProductName}'`
        : '';
}
